...
class SimpleWAF {
	...
	private function msgWarning($msg) {
		...
		<center>
			<img src=\"https://img.4plebs.org/boards/tg/image/1403/85/1403850647688.png\" width=\"150\">
			<br /><br />
			<i><b>[ ".$msg." ]</b></i>
		</center>";
		return $show;
	...
	public function secureMe($activate) {
		...
		if($activate == true) {
			...
			if ( $this->protect_xss === true ) {
				$ct_rules = array( '<script>','http\:\/\/', 'https\:\/\/', 'cmd=', '&cmd', 'exec', 'concat', './', '../',  'http:', 'h%20ttp:', 'ht%20tp:', 'htt%20p:', 'http%20:', 'https:', 'h%20ttps:', 'ht%20tps:', 'htt%20ps:', 'http%20s:', 'https%20:', 'ftp:', 'f%20tp:', 'ft%20p:', 'ftp%20:', 'ftps:', 'f%20tps:', 'ft%20ps:', 'ftp%20s:', 'ftps%20:', '.php?url=' );
				$check    = str_replace($ct_rules, '*', $this->get_query_string() );
